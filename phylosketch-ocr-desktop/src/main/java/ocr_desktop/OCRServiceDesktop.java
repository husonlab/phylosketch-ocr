/*
 *  OCRServiceDesktop.java Copyright (C) 2025 Daniel H. Huson
 *
 ...
 */
public class OCRServiceDesktop implements OCRService {
	 ...
	 * @return the words
	 */
	public List<OCRService.Word> getWords(Image image) throws IOException {
		if (tesseract == null) {
			tesseract = createAPI();
		}
	 ...
	 * @return the words
	 */
	private static List<OCRService.Word> getWords(String imagePath) throws IOException {
		var words = new ArrayList<OCRService.Word>();
					...
					do {
						try (IntPointer left = new IntPointer(1);
							...
							if (word != null) {
								...
								words.add(new OCRService.Word(text, confidence, new Rectangle2D(Math.min(left.get(), right.get()), Math.min(top.get(), bottom.get()), Math.abs(right.get() - left.get()), Math.abs(top.get() - bottom.get()))));
							}
						}
